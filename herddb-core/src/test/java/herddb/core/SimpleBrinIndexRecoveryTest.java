...
public class SimpleBrinIndexRecoveryTest {
    ...
    public void createRecoveryIndex_withrepeatedkey() throws Exception {
        ...
        try (DBManager manager = new DBManager("localhost",
            ...
            manager.start();
            assertTrue(manager.waitForTablespace("tblspace1", 10000));
            TranslatedQuery translated = manager.getPlanner().translate(TableSpace.DEFAULT, "SELECT * FROM tblspace1.t1 WHERE name='my_repeatad_key'", Collections.emptyList(), true, true, false);
