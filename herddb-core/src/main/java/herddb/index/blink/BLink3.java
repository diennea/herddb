...
public class BLink3<K extends Comparable<K>> {
    ...
    public static void main(String[] args) {
        ...
        try {
            while(!deadlock) {
                ...
                BLink3<Sized<Long>> tree = new BLink3<>(3,3);
...
//                long maxID = 10000;
    ...
    private static void insert(long element, BLink3<Sized<Long>> tree) {
    ...
    public BLink3(long nodeSize, long leafSize) {
...
//                        K searchKey = v;
                        ...
                        //  LOTHRUIN il lock di controllo genera deadlock!!?!?
//                        /* LOTHRUIN solo check di controllo si potraà dropppare */
//                        if (a.isLeaf()) {
//                            lock(current);
//
//                            a = get(current);
//
//                            /* If necessary */
//                            moveRight = moveRight(a, searchKey, current);
//                            t = moveRight.t;
//                            current = moveRight.current;
//                            a = moveRight.a;
//
//                            System.out.println(Thread.currentThread().getId() + " m Searching roots: a " + a + " current " + current );
//
//                            if (current.value != aprime.getPage()) {
//                                System.out.println(Thread.currentThread().getId() + " foglia sbagliata " + a
//                                        + " searchKey " + searchKey + " current " + current);
//                                throw new InternalError("foglia sbagliata!");
//                            }
//                            unlock(current);
//                        }
...
//                        while(a.getPage() != aprime.getPage()) {
//                            if (a.isLeaf() && a.getPage() != aprime.getPage()) {
//                                System.out.println(Thread.currentThread().getId() + " + Searching roots: a " + a + " current " + current );
//                                System.out.println(Thread.currentThread().getId() + " Raggiunta una fogliaaa! " + a + " searchKey " + searchKey);
//                                throw new InternalError("Raggiunta una fogliaaa!");
//                            }
//                            t = current;
//                            current = a.scanNode(searchKey);
//
...
//        locks.put(pointer.value, new ReentrantLock());
    ...
    private void put(BLinkNode<K> node, BlinkPtr pointer, boolean locked) {
        /* Crea il lock se non creato... questo perché attualmente il lock è creato a parte, meglio
         * sarebbe crearlo sulla pagina (fouri dai dati "scaricabili") e shararlo tra le versioni */
        locks.putIfAbsent(pointer.value, new ReentrantLock());
//        locks.put(pointer.value, new ReentrantLock());
